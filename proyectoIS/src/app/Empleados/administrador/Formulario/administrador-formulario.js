let boton = document.getElementById("btnAdministrador")
...
let registrarAdministrador = async()=>{
    ...
    try {
        ...
        if(!campos.correo){
            ...
            valid = false;
        }else if(await verificarCorreo(campos.correo,1)){
        ...
        if(!valid){
            ...
            // Aquí se registrará el administrador
            const peticion = await fetch('http://localhost:8085/api/administrador/save', {
                method: "POST",
                headers: {
...
function mostrarMensajeExito() {
    ...
    messageTitle.textContent = 'Administador registrado con éxito';
...
function mostrarMensajeError(mensaje) {
    ...
    //Crear el título del mensaje
    const messageTitle = document.createElement('h2');
    messageTitle.textContent = 'Error al registrar el administrador';
