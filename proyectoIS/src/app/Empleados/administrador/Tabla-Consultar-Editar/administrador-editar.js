document.addEventListener("DOMContentLoaded", async() => {
    ...
    if(id){
        const administrador = await obtenerAdministrador(id);
        llenarFormulario(administrador);
    }

    document.getElementById("btnGuardarCambios").addEventListener("click", async(event) => {
        event.preventDefault();
        const administrador = await obtenerAdministrador(id);
        await guardarCambios(administrador, id);
    });
})
...
function llenarFormulario(administrador){
    document.getElementById("nombre").value = administrador.nombre;
    document.getElementById("cedula").value = administrador.cedula;
    document.getElementById("sexo").value = administrador.sexo;
    document.getElementById("edad").value = administrador.edad;
    document.getElementById("nacionalidad").value = administrador.nacionalidad;
    document.getElementById("email").value = administrador.correo;
    
    const telf = administrador.telefono;
    ...
    document.getElementById("rol").value = administrador.rol;
    document.getElementById("sueldo").value = administrador.sueldo;
    document.getElementById("stLaboral").value = administrador.stlaboral;
    document.getElementById("experiencia").value = administrador.experiencia;
    document.getElementById("especializacion").value = administrador.especializacion;
}
...
async function guardarCambios(administrador,id){
    if(await mostrarMensajeConfirmacion()){
        try{
            ...
            if(parseInt(document.getElementById("cedula").value) === administrador.cedula){
            ...
            if(document.getElementById("email").value === administrador.correo){
            ...
            if(document.getElementById("num").value + "-" + document.getElementById("telefono").value === administrador.telefono){
            ...
            campos.especializacion = document.getElementById("especializacion").value;
            campos.username = administrador.username;
            campos.password = administrador.password;
            ...
            if(!valid){
                ...
                const respuesta = await fetch(`http://localhost:8085/api/administrador/update/${id}`, {
                    method: 'PUT',
                    headers: {
                ...
                if(respuesta.ok){
                    ...
                    mostrarMensajeError('Ha ocurrido un error al modificar los datos del Administrador.');
                }
            }
        }catch(error){
        ...
        window.location.href = '../Tabla-Consultar-Editar/administrador-tabla.html'; // Redirigir a la tabla
    }
...
function mostrarMensajeExito() {
    ...
    messageTitle.textContent = 'Se han modificado los datos del Administrador con éxito.';
...
function mostrarMensajeError(mensaje) {
    ...
    //Crear el título del mensaje
    const messageTitle = document.createElement('h2');
    messageTitle.textContent = 'Error al modificar los datos de Administrador';
